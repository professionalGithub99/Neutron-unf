...
//the column count is the number of energyBins
    ...
    simulatedBinSums.resize(unfold_pnt->eBins);
...
Eigen::VectorXf UNCERT::generateFakeData(){
    ...
    Eigen::VectorXf temp=UNCERT::scaleVectorOverOne(UNCERT::unfold_pnt->getLightOutputResponse());
    unfold_pnt->setLightOutput(temp);
    UNCERT::lightOutputs= unfold_pnt->getLightOutputResponse();
...
cout<<simulatedBinUncertainties<<"sim bin ucner";
