...
TGraph* UNFOLD::plotAnswer()
{
  ...
  cout << hStart << " " << hEnd << " " << m_fio->nBins << endl;
...
void UNFOLD::plotProjection()
{
  //new TCanvas;
  backProject();
  hFinal = new TH1F("Proj","Projection;Light Output (MeVee);Counts",m_fio->nBins,0,m_fio->endR);
  for(int bN=0; bN<m_fio->nBins; bN++){
...
vector<float> UNFOLD::getProjectionErrors(){
